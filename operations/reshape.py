...
class Reshape(Operator):
    ...
    def forward(self, input: torch.Tensor, shape=None):
        # print('input:', input)
        # print('inpus shape:', input.shape)
        # print('reshape size:', shape)
        ...
        if not self.enable_pruning:
            ...
            # print('new shape:', tuple(shape))

            # return torch.reshape(input, tuple(shape))

            # TODO: YOLOX-ViT reshapes
            if input.shape == (169, 128):
                return torch.reshape(input, (169, -1, 128))
            
            if input.shape[1:] == (128, 169):
                return torch.reshape(input, (-1, 128, 13, 13))
            
            if input.shape[0] == 169 and input.shape[2] == 128:
                return torch.reshape(input, (169, -1, 128))

            if input.shape == (169, 4, 32):
                return torch.reshape(input, (169, 128))
            
            if input.shape[1:] == (128, 13, 13):
                return torch.reshape(input, (-1, 128, 169))


            # TODO: look into this, why I need to hardcode the dimensions to make this work
            # TODO: why this reshape function takes the wrong reshape size (takes the same reshape size as the first reshape block)
            # seems like this was because one setting in the yolox.yaml exp file => onnx_quirks: "{'Reshape': {'fix_batch_size': True}}"
            # removing this setting fixes this problem
            if input.shape[3] == 52:
                return torch.reshape(input, (-1, 7, 2704))
            elif input.shape[3] == 26:
                return torch.reshape(input, (-1, 7, 676))
            elif input.shape[3] == 13:
                return torch.reshape(input, (-1, 7, 169))
            else:
                return torch.reshape(input, tuple(shape))
