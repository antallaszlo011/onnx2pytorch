            ...
            for in_op_id in node.input:
                if in_op_id in still_needed_by:
                    ...
                    if len(still_needed_by[in_op_id]) == 0:
                        if in_op_id in activations and in_op_id != 'images':    # TODO: I added the -- and in_op_id != 'images' -- subcondition
                            del activations[in_op_id]

            if self.debug:
                # TODO: KeyError: 'images' not found in "activations"
                # print('Activations:', activations.keys())
                # print('Self input names:', self.input_names)
                # print('Node input names:', node.input)
