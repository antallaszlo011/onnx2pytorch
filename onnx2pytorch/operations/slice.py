...
class Slice(nn.Module):
    ...
    def _fixup_params(self, shape, start, end, axes, steps):
        # print('shape, start, end, axes, steps:', shape, start, end, axes, steps)
    ...
    def forward(
        ...
        # TODO: note that this implementation is not a general implementation of the ONNX.Slice operation
        # This implementation only works when there is only 1 axes in which slicing happens, however, ONNX.Slice would support multi-dimensional slicing, i.e., slicing in multiple axes
        # extending this method is possible, however we don't need it for YOLO-X at the moment
        ...
        # print('Slice parameters:', starts, ends, axes, steps)
        ...
        # !!! my implementation !!!
        # need to consider a general solution ... 

        slc = []
        for i in range(len(shape)):
            if i == axes:
                slc.append(slice(start, end, steps))
            else:
                slc.append(slice(0, shape[i], 1))

        # print('Slice shape:', x[slc].shape)
        return x[slc]
